...
public class AddAgendaHolidayServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String holidayType = json.getString("type");

        String sql = "INSERT INTO agenda (title, start_date, end_date, event) VALUES (?, ?, ?, ?)";

        try (java.sql.Connection conn = DBConnection.connect(); java.sql.PreparedStatement stmt = conn.prepareStatement(sql)) {
            
            stmt.setString(1, title);
            stmt.setDate(2, java.sql.Date.valueOf(startDate));
            stmt.setDate(3, java.sql.Date.valueOf(endDate));
            stmt.setString(4, holidayType);
            stmt.executeUpdate();
